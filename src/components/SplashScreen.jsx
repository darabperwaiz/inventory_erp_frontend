...
export default function SplashScreen({ onComplete }) {
  ...
  useEffect(() => {
    ...
    // Initialize all paths: no fill, stroke only, no dash (use style to beat CSS specificity)
    pathEls.forEach(el => {
    ...
    // Blue square gets blue stroke
    pathEls[6].style.stroke = '#0047FF';

    // Get lengths after attributes are set
    const paths = Array.from(pathEls).map(el => ({
      el,
    ...
    // Set initial state — dasharray uses path length, animation scales via dashoffset
    paths.forEach(({ el, len }) => {
      el.style.transition = 'none';
      el.style.strokeDasharray = '20 ' + len;
      el.style.strokeDashoffset = '0';
    });
    ...
    // Phase 3: Fill reveal (3s)
    const t3 = setTimeout(() => {
      paths.forEach(({ el }, i) => {
        el.style.fill = PATHS[i].fill;
